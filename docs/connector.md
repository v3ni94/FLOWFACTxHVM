...
| Header | Standard: `cognitoToken` (automatisch getauschtes Sitzungstoken, siehe unten), `Accept-Language: de`, `Accept: application/json`; `x-ff-version` nur wo dokumentiert; `x-ff-company-id` nur wenn gesetzt. Zur Diagnose umschaltbar auf den ungetauschten Zugangsschlüssel in den Kopfzeilen `x-ff-api-token`, `token`, `Authorization: Bearer` oder `x-api-key` (Adminbereich, "Übertragungsform des Tokens") |
| Token-Tausch | Der in den Einstellungen hinterlegte Zugangsschlüssel ist kein Aufruftoken. `App\Flowfact\Client\CognitoTokenCache` tauscht ihn über `GET admin-token-service/public/adminUser/authenticate` (Kopfzeile `token`) gegen ein Cognito-Token, das rund 30 Minuten gültig ist. Das Ergebnis wird bis kurz vor Ablauf zwischengespeichert (Schlüssel je Zugangsschlüssel), ein gleichzeitiger doppelter Tausch ist harmlos und wird deshalb nicht gesperrt. `TokenScrubber` entfernt sowohl den Zugangsschlüssel als auch ein zwischengespeichertes Cognito-Token aus Protokoll und Fehlermeldungen (Prüfbericht 2026-09-21); die Diagnose im Adminbereich erzwingt vor jedem Lauf einen frischen Tausch, statt ein zwischengespeichertes Token stillschweigend wiederzuverwenden |
