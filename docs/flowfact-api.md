...
1. Wie ein API-Token in der FLOWFACT-Oberfläche erzeugt wird (Menüpfad, Berechtigung). Das SDK zeigt nur die Endpunkte aus 3.4, nicht die UI.
   **Geklärt, Stand 21.09.2026.** Erzeugt unter apps.flowfact.com, Einstellungen, Tools & Integration, API-Zugänge. Der dort angezeigte Zugangsschlüssel hat UUID-Form und ist selbst **kein** Aufruftoken: er ist der "platformToken" aus Punkt 5. Erster Kontakt vom Konto der Hausverwaltung Müller GmbH: alle Dienste antworteten mit HTTP 403 `Credentials invalid`, solange der Schlüssel unverändert als `x-ff-api-token`, `token`, `Authorization: Bearer` oder `x-api-key` gesendet wurde (App\Flowfact\Client\TokenHeader probiert seitdem alle vier Formen zur Diagnose). Ursache und Lösung: developers.flowfact.com/api dokumentiert den Tausch gegen ein Cognito-Token (siehe Punkt 5); erst dieses Cognito-Token authentisiert die Dienste. Müller FLOW tauscht seit App\Flowfact\Client\CognitoTokenCache automatisch und sendet das Ergebnis als Kopfzeile `cognitoToken`; die ungetauschten Formen bleiben als manuelle Diagnose- und Rückfalloption erhalten (Adminbereich, "Übertragungsform des Tokens").
...
5. Was ein "platformToken" bei `GET /public/adminUser/authenticate` ist und ob dieser Weg für Dritte offen steht. Quelle: src/service/AdminTokenService/PublicAdminUserController.js:12-24
   **Geklärt, Stand 21.09.2026** über developers.flowfact.com/api ("How to use the FLOWFACT-API"): der platformToken ist genau der in den API-Zugängen erzeugte Zugangsschlüssel. Der Weg steht für Dritte offen und ist der von FLOWFACT dokumentierte Integrationsweg ("If you want to customize a FLOWFACT-system, you need to authenticate against the services... you need a cognito-token. To get this token, you need to make an API-Call where you have to provide the userplatform-token of the account."). Aufruf: `GET admin-token-service/public/adminUser/authenticate` mit Kopfzeile `token: <Zugangsschlüssel>`, keine weitere Authentisierung. Antwort: der rohe JWT-Text (laut Beispiel ohne Anführungszeichen, App\Flowfact\Client\CognitoTokenCache akzeptiert zusätzlich eine JSON-Zeichenkette), gültig laut Dokumentation "approx. 30 min", danach erneut zu tauschen. Das Cognito-Token wird als Kopfzeile `cognitoToken` bei allen weiteren Aufrufen gesendet (deckt sich mit Abschnitt 3.1 und 3.3). Müller FLOW liest die Gültigkeit aus dem `exp`-Anspruch des Tokens und tauscht rechtzeitig vor Ablauf erneut, mit einer Minute Sicherheitsabstand.
