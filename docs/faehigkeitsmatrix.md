...
| --- | --- | --- | --- | --- | --- |
| Authentifizierung per API-Token | SDK: src/http/..., Header `x-ff-api-token` (flowfact-api.md 3.1, 3.2) | Header `x-ff-api-token`, `Accept-Language: de`, optional `x-ff-company-id` bei jedem Aufruf | gültiger API-Token des Kontos | simuliert mit Http::fake (FlowfactClientTest, TokenLeakTest) | Ob der Header extern für alle Services akzeptiert wird, ist offen (Abschnitt 9, Punkt 2); Token wird nie protokolliert |
